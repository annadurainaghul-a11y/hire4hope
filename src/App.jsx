...
:root {
  ...
  --shadowMd: 0 6px 24px rgba(0,0,0,.1);
  --hero:     linear-gradient(150deg, #2d1e08 0%, #4a3010 25%, #3d6010 55%, #4f7c1f 80%, #2d4a0a 100%);
}
...
@keyframes pring    { 0%{ box-shadow:0 0 0 0 rgba(79,124,31,.4) } 70%{ box-shadow:0 0 0 20px rgba(79,124,31,0) } 100%{ box-shadow:0 0 0 0 rgba(79,124,31,0) } }
...
.btn::after {
  ...
  left:-80%; width:60%; transition:left .5s; pointer-events:none; z-index:2;
}
...
.ch:hover { transform:translateY(-3px); box-shadow:0 8px 28px rgba(0,0,0,.11) !important }
.rh { transition:all .14s !important }
.rh:hover { transform:translateX(3px) }
...
/* ── Form controls — always light, always readable ── */
...
input:focus, textarea:focus, select:focus {
  ...
  box-shadow: 0 0 0 3px rgba(79,124,31,.12) !important;
...
::-webkit-scrollbar-thumb { background: rgba(79,124,31,.3); border-radius: 10px }

/* ── Responsive ── */
@media(max-width:768px) {
  .dl { flex-direction:column !important }
  .ds { width:100% !important; max-height:300px; border-right:none !important; border-bottom:1px solid var(--border) }
  .dd { padding:14px !important }
  .g2 { grid-template-columns:1fr !important }
  .ht { font-size:30px !important }
  .fc { padding:20px 14px !important; margin-top:-20px !important }
...
function SuccessOverlay({name,onDone}){
  ...
  return(<><Confetti/><div onClick={onDone} style={{position:"fixed",inset:0,zIndex:9998,background:"linear-gradient(135deg,rgba(4,20,2,.97),rgba(8,40,4,.96))",display:"flex",alignItems:"center",justifyContent:"center",animation:"ovIn .4s ease both",cursor:"pointer"}}><div style={{textAlign:"center",padding:"0 32px",maxWidth:520}}>
    <div className="fl" style={{width:110,height:110,borderRadius:"50%",background:"linear-gradient(135deg,#1a4a10,#2d7a1a,#5a8a2e)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 28px",animation:"popIn .7s cubic-bezier(.34,1.56,.64,1) both,pring 2.5s ease-out .9s",boxShadow:"0 0 0 20px rgba(79,124,31,.1),0 20px 60px rgba(79,124,31,.55)"}}>
    ...
    <p style={{color:"var(--green)",fontSize:16,lineHeight:1.85,marginBottom:20,animation:"txtUp .5s ease .6s both",fontWeight:300}}>We received your application and a confirmation email is on its way. Track your status anytime in "My Status". ☕</p>
    <p style={{color:"var(--txtMute)",fontSize:12,animation:"txtUp .5s ease .9s both"}}>Tap anywhere to close</p>
  </div></div></>);
}
...
function Chatbot({ctx="apply"}){
  ...
  const BG="linear-gradient(165deg,#3a6a18,#1e4008)";
  return(<>
    <div style={{position:"fixed",bottom:24,right:24,zIndex:1000}}>
      <button type="button" onClick={()=>setOpen(o=>!o)} className="btn" style={{width:60,height:60,borderRadius:"50%",background:BG,boxShadow:"0 8px 32px rgba(79,124,31,.55)",display:"flex",alignItems:"center",justifyContent:"center",animation:wig&&!open?"wiggle .55s ease":"none"}}>
      ...
      {!open&&<div style={{position:"absolute",top:0,right:0,width:18,height:18,borderRadius:"50%",background:"linear-gradient(135deg,#4a7a22,#5a8a2e)",border:"2px solid #022c22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:"#fff",fontWeight:800}}>AI</div>}
    </div>
    {open&&(<div style={{position:"fixed",bottom:96,right:24,zIndex:999,width:"min(360px,calc(100vw - 32px))",maxHeight:500,background:"#0c1a0e",borderRadius:20,overflow:"hidden",boxShadow:"0 24px 80px rgba(0,0,0,.55)",display:"flex",flexDirection:"column",animation:"chatPop .38s cubic-bezier(.34,1.56,.64,1) both",border:"1px solid #b8d888"}}>
      <div style={{background:BG,padding:"14px 18px",display:"flex",alignItems:"center",gap:12}}>
        <div className="fl" style={{width:38,height:38,borderRadius:"50%",background:"rgba(79,124,31,.15)",border:"1.5px solid rgba(79,124,31,.3)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Logo s={22}/></div>
        <div><div style={{fontFamily:"'Poppins',sans-serif",fontSize:16,color:"#e8f5d0",fontWeight:700,letterSpacing:"-.2px"}}>Brew</div><div style={{color:"var(--green)",fontSize:11,display:"flex",alignItems:"center",gap:5,marginTop:1}}><span style={{width:6,height:6,borderRadius:"50%",background:"#5a8a2e",boxShadow:"0 0 6px #5a8a2e",display:"inline-block"}}/>AI · Hope Coffee</div></div>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"12px 12px 6px",display:"flex",flexDirection:"column",gap:10,minHeight:0}}>
        {msgs.map((m,i)=>(<div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start",animation:"msgIn .25s ease both"}}>
          ...
          <div style={{maxWidth:"78%",padding:"9px 13px",borderRadius:m.role==="user"?"18px 18px 4px 18px":"18px 18px 18px 4px",background:m.role==="user"?"linear-gradient(135deg,#2d6018,#3d7a20)":"rgba(255,255,255,.07)",color:m.role==="user"?"#fff":"#d4edaa",fontSize:13,lineHeight:1.65,border:m.role==="assistant"?"1px solid rgba(79,124,31,.1)":"none"}}>
      ...
      {msgs.length<=1&&<div style={{padding:"0 12px 8px",display:"flex",gap:6,flexWrap:"wrap"}}>{(ctx==="status"?["What does my status mean?","When will I hear back?"]:["How do I apply?","What positions are open?","What's the pay?"]).map(q=><button key={q} type="button" onClick={()=>setInp(q)} className="btn" style={{padding:"5px 11px",borderRadius:18,border:"1px solid rgba(79,124,31,.2)",background:"rgba(79,124,31,.06)",color:"var(--green)",fontSize:11,fontWeight:600}}>{q}</button>)}</div>}
      <div style={{padding:"8px 12px 12px",borderTop:"1px solid rgba(255,255,255,.05)",background:"rgba(0,0,0,.12)"}}>
        <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
          <textarea value={inp} onChange={e=>setInp(e.target.value.slice(0,800))} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();}}} placeholder="Ask Brew anything…" rows={1} style={{flex:1,resize:"none",border:"1px solid rgba(79,124,31,.2)",borderRadius:12,padding:"9px 12px",fontSize:13,color:"#d4edaa",outline:"none",background:"rgba(255,255,255,.06)",maxHeight:80,overflowY:"auto"}}/>
...
/* ─── FORM HELPERS ────────────────────────────────────────────────────── */
const FL=({text,opt})=>(<div style={{color:"#8abb4a",fontSize:10,fontWeight:600,marginBottom:6,letterSpacing:".08em",textTransform:"uppercase"}}>{text}{opt&&<span style={{textTransform:"none",fontWeight:400,color:"#999",marginLeft:5}}>(optional)</span>}</div>);
const FD=({label})=>(<div style={{display:"flex",alignItems:"center",gap:12,margin:"4px 0 16px"}}><div style={{flex:1,height:1,background:"linear-gradient(90deg,#c8dfa0,transparent)"}}/><span style={{fontFamily:"'Poppins',sans-serif",fontSize:15,fontWeight:700,color:"#8abb4a",padding:"3px 14px",background:"rgba(79,124,31,.12)",borderRadius:24,border:"1px solid rgba(79,124,31,.3)"}}>{label}</span><div style={{flex:1,height:1,background:"linear-gradient(90deg,transparent,#c8dfa0)"}}/></div>);
...
function ApplyPage({onSubmit}){
  ...
  const IS=n=>({width:"100%",background:"#fff",border:`1.5px solid ${errs[n]?"#ef4444":foc===n?"var(--green)":"var(--border)"}`,borderRadius:12,padding:"12px 16px",color:"#1a1a1a",fontSize:14,outline:"none",boxShadow:foc===n?"0 0 0 3px rgba(79,124,31,.12)":"none",transition:"all .2s"});
  ...
  return(<>
    ...
    <div style={{minHeight:"100%",background:"#f5f0e8"}}>
      {/* Hero */}
      <div style={{position:"relative",padding:"52px 24px 72px",overflow:"hidden",background:"var(--hero)",backgroundSize:"400% 400%",animation:"bgFlow 16s ease infinite"}}>
        <div style={{position:"absolute",top:-60,right:-60,width:300,height:300,borderRadius:"50%",background:"radial-gradient(circle,rgba(79,124,31,.14),transparent 70%)",pointerEvents:"none"}}/>
        <div style={{position:"relative",zIndex:2,maxWidth:580,margin:"0 auto"}}>
          <div className={mounted?"fu":""} style={{display:"flex",alignItems:"center",gap:18,marginBottom:28}}>
            <div className="fl" style={{background:"rgba(255,255,255,.18)",borderRadius:20,padding:"12px 14px",backdropFilter:"blur(12px)",border:"1px solid rgba(255,255,255,.25)"}}><Logo s={54}/></div>
            <div><div style={{fontFamily:"'Poppins',sans-serif",color:"var(--green)",fontSize:16,fontWeight:700,marginBottom:2}}>Now Hiring at</div><div style={{fontFamily:"'Poppins',sans-serif",fontSize:22,color:"#e8f5d0",fontWeight:700,letterSpacing:"-.3px"}}>Hope Coffee Melissa</div></div>
          </div>
          ...
          <p className={mounted?"fu":""} style={{color:"rgba(255,255,255,.85)",fontSize:16,margin:"0 0 28px",lineHeight:1.8,maxWidth:460,fontWeight:300,animationDelay:"130ms"}}>Melissa's gathering place — craft coffee, good people, second chances. Every application is read by a real human.</p>
          <div className={mounted?"fu":""} style={{display:"flex",gap:10,flexWrap:"wrap",animationDelay:"190ms"}}>
            {["Welcoming team","$11–13/hr","Flexible hours","Community-driven"].map(t=><span key={t} style={{background:"rgba(255,255,255,.1)",color:"var(--txtMid)",fontSize:12,padding:"6px 16px",borderRadius:24,fontWeight:500,border:"1px solid rgba(255,255,255,.15)"}}>✓ {t}</span>)}
          </div>
        </div>
      </div>
      {/* Form */}
      <div style={{maxWidth:580,margin:"0 auto",padding:"0 16px 80px"}}>
        <div className={`${mounted?"si":""} fc`} style={{background:"#fff",borderRadius:24,padding:"36px 32px",marginTop:-36,boxShadow:"var(--shadowMd)",border:"1px solid var(--border)"}}>
          <div style={{background:"rgba(79,124,31,.06)",borderRadius:12,padding:"14px 18px",marginBottom:24,border:"1px solid #b8d888"}}><p style={{color:"var(--txtMid)",fontSize:13,lineHeight:1.75,margin:0}}><strong>🔒 Your privacy matters.</strong> Job-related info only. You'll receive a confirmation email and notifications when your status changes.</p></div>
              ...
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}} className="g2">
                  {[
                    ...
                    <div key={opt.v} onClick={()=>up("pos",opt.v)} style={{borderRadius:14,padding:"14px 16px",cursor:"pointer",border:`2px solid ${f.pos===opt.v?"rgba(79,124,31,.7)":"rgba(255,255,255,.1)"}`,background:f.pos===opt.v?"rgba(79,124,31,.14)":"rgba(255,255,255,.03)",transition:"all .18s",position:"relative"}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:6}}>
                        ...
                        <span style={{color:f.pos===opt.v?"#fff":"#1a1a1a",fontSize:14,fontWeight:700}}>{opt.v}</span>
                      </div>
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:7}}>
                        <span style={{background:"rgba(79,124,31,.12)",color:"var(--green)",fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:10,border:"1px solid rgba(79,124,31,.2)"}}>{opt.pay}</span>
                        <span style={{color:f.pos===opt.v?"rgba(255,255,255,.7)":"#999",fontSize:10,fontWeight:500,alignSelf:"center"}}>{opt.hrs}</span>
                      </div>
                      <p style={{color:f.pos===opt.v?"rgba(255,255,255,.85)":"#666",fontSize:12,lineHeight:1.6,margin:0}}>{opt.desc}</p>
                    </div>
                  ))}
                </div>
                {EL("pos")}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <FL text="Resume"/>
                  <div style={{display:"flex",gap:3,background:"rgba(255,255,255,.06)",borderRadius:24,padding:3,border:"1px solid rgba(255,255,255,.1)"}}>
                    {["text","upload"].map(tab=><button key={tab} type="button" onClick={()=>setRTab(tab)} className="btn" style={{padding:"5px 14px",borderRadius:18,border:"none",fontSize:12,fontWeight:rTab===tab?700:500,background:rTab===tab?BG:"transparent",color:rTab===tab?"#fff":"rgba(79,124,31,.6)",transition:"all .2s"}}>{tab==="text"?"✏️ Write":"📎 Upload"}</button>)}
                  </div>
                </div>
                {rTab==="text"
                  ...
                  :<div onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);if(e.dataTransfer.files[0])pickFile(e.dataTransfer.files[0]);}} onClick={()=>fRef.current?.click()} style={{width:"100%",minHeight:130,borderRadius:14,border:`2px dashed ${drag?"rgba(79,124,31,.7)":rName?"rgba(79,124,31,.5)":"rgba(255,255,255,.12)"}`,background:drag?"rgba(79,124,31,.08)":rName?"rgba(79,124,31,.05)":"rgba(255,255,255,.02)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,cursor:"pointer",padding:"18px 16px",transition:"all .2s"}}>
                    ...
                    {rName
                      ?<><div style={{width:44,height:44,borderRadius:"50%",background:"linear-gradient(135deg,#065f46,#4a7a22)",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg></div><p style={{color:"var(--green)",fontSize:13,fontWeight:700,margin:0}}>{rName}</p><p style={{color:"var(--txtMute)",fontSize:11,margin:0}}>Click to replace · AI will read this file</p></>
                      :<><div style={{width:44,height:44,borderRadius:"50%",background:"rgba(79,124,31,.08)",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#5a8a2e" strokeWidth="2" strokeLinecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg></div><p style={{color:"var(--txtMid)",fontSize:13,fontWeight:600,margin:0}}>Drag & drop or click to upload</p><p style={{color:"var(--txtMute)",fontSize:11,margin:0}}>PDF, Word, or .txt · max 800KB</p></>}
              ...
              <div><FL text="Online presence" opt/><input type="text" value={f.fp} onChange={e=>up("fp",e.target.value.slice(0,300))} placeholder="LinkedIn URL or portfolio link" style={IS("fp")} onFocus={()=>setFoc("fp")} onBlur={()=>setFoc(null)}/><span style={{color:"var(--txtMute)",fontSize:11,marginTop:4,display:"block"}}>Tip: a real link improves your fit score.</span></div>
            </div>
            <button type="button" onClick={submit} disabled={busy} className="btn" style={{marginTop:4,background:busy?"rgba(79,124,31,.1)":BG,color:busy?"rgba(144,187,74,.4)":"#fff",border:"none",borderRadius:14,padding:"16px 24px",fontSize:15,fontWeight:700,cursor:busy?"not-allowed":"pointer",boxShadow:busy?"none":"0 8px 32px rgba(79,124,31,.5)",display:"flex",alignItems:"center",justifyContent:"center",gap:10,width:"100%"}}>
              {busy?<><div style={{width:16,height:16,borderRadius:"50%",border:"2px solid rgba(79,124,31,.3)",borderTopColor:"#5a8a2e",animation:"spin .7s linear infinite"}}/>{bMsg}</>:"Submit My Application →"}
            </button>
          </div>
        </div>
      </div>
...
function StatusPage({applicants}){
  ...
  const SI={
    New:     {icon:"📬",label:"Application Received",desc:"We've got your application and it's in our review queue. Brian personally reviews every application.",color:"rgba(96,165,250,.8)",bg:"rgba(59,130,246,.1)",bd:"rgba(59,130,246,.25)"},
    ...
    Hired:   {icon:"🎉",label:"Offer Extended!",desc:"Congratulations! Welcome to the Hope Coffee family. Check your email for onboarding details.",color:"var(--txtMid)",bg:"rgba(79,124,31,.1)",bd:"rgba(79,124,31,.25)"},
  ...
  return(<div style={{minHeight:"100%",background:"#f5f0e8"}}>
    <div style={{position:"relative",padding:"44px 24px 56px",overflow:"hidden",background:"var(--hero)",backgroundSize:"400% 400%",animation:"bgFlow 16s ease infinite"}}>
      <div style={{position:"relative",zIndex:2,maxWidth:560,margin:"0 auto"}}>
        <div className={mounted?"fu":""} style={{display:"flex",alignItems:"center",gap:14,marginBottom:20}}>
          ...
          <div><div style={{fontFamily:"'Poppins',sans-serif",fontSize:24,color:"#e8f5d0",fontWeight:700}}>My Application</div><div style={{fontFamily:"'Poppins',sans-serif",color:"var(--green)",fontSize:15,marginTop:2}}>Hope Coffee Melissa</div></div>
        </div>
    ...
    <div style={{maxWidth:580,margin:"0 auto",padding:"0 16px 80px"}}>
      {stage==="email"&&<div className={mounted?"si":""} style={{background:"#fff",borderRadius:20,padding:"24px",marginTop:-24,boxShadow:"0 20px 60px rgba(0,0,0,.12)",marginBottom:14,border:"1px solid var(--border)"}}>
        <div style={{fontFamily:"'Poppins',sans-serif",fontSize:15,color:"var(--green)",fontWeight:700,marginBottom:12}}>🔍 Look up your application</div>
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          <input type="email" value={emailIn} onChange={e=>setEmailIn(e.target.value.slice(0,200))} onKeyDown={e=>{if(e.key==="Enter")lookup();}} placeholder="your@email.com" style={{flex:"1 1 200px",background:"rgba(255,255,255,.03)",border:"1.5px solid rgba(79,124,31,.2)",borderRadius:12,padding:"12px 16px",color:"#1a1a1a",fontSize:14,outline:"none"}}/>
          <button type="button" onClick={lookup} disabled={sending} className="btn" style={{padding:"12px 22px",borderRadius:12,background:BG,color:"#fff",fontSize:14,fontWeight:700,boxShadow:"0 4px 18px rgba(79,124,31,.4)",whiteSpace:"nowrap",flexShrink:0}}>
      ...
      {stage==="verify"&&<div className="si" style={{background:"#fff",borderRadius:20,padding:"28px",marginTop:-24,boxShadow:"0 20px 60px rgba(0,0,0,.12)",border:"1px solid var(--border)"}}>
        <div style={{textAlign:"center",marginBottom:20}}><div style={{fontSize:44,marginBottom:10}}>📧</div><h3 style={{fontFamily:"'Poppins',sans-serif",fontSize:22,color:"#e8f5d0",fontWeight:700,margin:"0 0 8px"}}>Check your email</h3><p style={{color:"var(--green)",fontSize:13,lineHeight:1.7}}>We sent a 6-digit code to <strong>{emailIn}</strong></p></div>
        <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:12}}>
          <input type="text" value={codeIn} onChange={e=>setCodeIn(e.target.value.replace(/\D/g,"").slice(0,6))} onKeyDown={e=>{if(e.key==="Enter")verify();}} placeholder="000000" maxLength={6} style={{flex:"1 1 160px",background:"rgba(255,255,255,.03)",border:`1.5px solid ${codeErr?"#ef4444":"rgba(79,124,31,.2)"}`,borderRadius:12,padding:"14px 16px",color:"#1a1a1a",fontSize:24,fontWeight:700,letterSpacing:"0.3em",textAlign:"center",outline:"none"}}/>
        ...
        <button type="button" onClick={reset} style={{background:"none",border:"none",color:"#999",fontSize:12,cursor:"pointer",textDecoration:"underline"}}>Use a different email</button>
      </div>}
      {stage==="notfound"&&<div className="fu" style={{background:"#fff",borderRadius:18,padding:"28px",border:"1px solid #fecaca",textAlign:"center",marginTop:-24}}>
        ...
        <p style={{color:"var(--green)",fontSize:13,margin:"0 0 18px",lineHeight:1.7}}>We couldn't find an application with that email. Double-check it or head to Apply to submit one.</p>
        <button type="button" onClick={reset} className="btn" style={{background:"rgba(79,124,31,.08)",border:"1px solid rgba(79,124,31,.2)",borderRadius:10,padding:"8px 20px",color:"var(--green)",fontSize:13,fontWeight:600,cursor:"pointer"}}>Try again</button>
      </div>}
      {stage==="found"&&found&&(()=>{
        ...
        return(<div className="fu" style={{display:"flex",flexDirection:"column",gap:12}}>
          <div style={{background:info.bg,borderRadius:20,padding:"22px 20px",border:`1px solid ${info.bd}`}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:16}}>
              ...
              <div><div style={{color:info.color,fontSize:11,fontWeight:800,letterSpacing:".12em",textTransform:"uppercase",marginBottom:5}}>Current Status</div><h2 style={{fontFamily:"'Poppins',sans-serif",fontSize:24,color:"#e8f5d0",fontWeight:700,margin:"0 0 8px"}}>{info.label}</h2><p style={{color:"var(--txtMid)",fontSize:13,lineHeight:1.75,margin:0}}>{info.desc}</p></div>
            </div>
          </div>
          {found.status!=="Rejected"&&<div style={{background:"#fff",borderRadius:18,padding:"18px 20px",border:"1px solid var(--border)"}}>
            <div style={{fontFamily:"'Poppins',sans-serif",fontSize:14,fontWeight:700,color:"var(--green)",marginBottom:14}}>Application Progress</div>
            <div style={{display:"flex",alignItems:"center"}}>
              {steps.map((step,i)=>{const act=idx>=i;const cur=idx===i;const sg=stG(step);return(<div key={step} style={{display:"flex",alignItems:"center",flex:i<steps.length-1?1:"none"}}>
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5}}>
                  <div style={{width:36,height:36,borderRadius:"50%",background:act?sg.g:"rgba(255,255,255,.05)",display:"flex",alignItems:"center",justifyContent:"center",animation:cur?"pulse2 2s ease infinite":"none",border:`2px solid ${cur?sg.d:"#ddd"}`}}>
          ...
          <div style={{background:"#fff",borderRadius:18,padding:"18px 20px",border:"1px solid var(--border)"}}>
            <div style={{fontFamily:"'Poppins',sans-serif",fontSize:14,fontWeight:700,color:"var(--green)",marginBottom:14}}>👤 Your Details</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}} className="g2">
              {[{l:"Name",v:found.full_name},{l:"Email",v:found.email},{l:"Phone",v:found.phone},{l:"Position",v:found.position},{l:"Applied",v:new Date(found.created_at).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})},{l:"Experience",v:`${found.experience_years} yr${found.experience_years!==1?"s":""}`}].map(x=><div key={x.l} style={{background:"#fafafa",borderRadius:10,padding:"10px 13px",border:"1px solid var(--border)"}}>
                <div style={{color:"#888",fontSize:10,fontWeight:700,letterSpacing:".08em",textTransform:"uppercase",marginBottom:3}}>{x.l}</div>
                <div style={{color:"#1a1a1a",fontSize:13,fontWeight:600,wordBreak:"break-word"}}>{x.v||"—"}</div>
              </div>)}
            </div>
          </div>
          <button type="button" onClick={reset} className="btn" style={{background:"transparent",border:"1px solid var(--border)",borderRadius:12,padding:"10px",color:"#888",fontSize:13,fontWeight:600,cursor:"pointer",textAlign:"center",width:"100%"}}>← Search a different email</button>
        </div>);
      })()}
    </div>
    <Chatbot ctx="status"/>
...
function AboutPage(){
  ...
  return(<div style={{minHeight:"100%",background:"#f5f0e8"}}>
    <div style={{position:"relative",padding:"52px 24px 72px",overflow:"hidden",background:"var(--hero)",backgroundSize:"400% 400%",animation:"bgFlow 16s ease infinite",backgroundSize:"400% 400%",animation:"bgFlow 14s ease infinite"}}>
      <div style={{position:"relative",zIndex:2,maxWidth:600,margin:"0 auto"}}>
        <div className={mounted?"fu":""} style={{display:"flex",alignItems:"center",gap:18,marginBottom:24}}>
          ...
          <div><div style={{fontFamily:"'Poppins',sans-serif",color:"var(--green)",fontSize:16,fontWeight:700,marginBottom:2}}>About</div><div style={{fontFamily:"'Poppins',sans-serif",fontSize:28,color:"#e8f5d0",fontWeight:700}}>Hope Coffee Melissa</div></div>
        </div>
        ...
        <p className={mounted?"fu":""} style={{color:"rgba(255,255,255,.85)",fontSize:15,lineHeight:1.85,maxWidth:480,fontWeight:300,animationDelay:"120ms"}}>Our mission is to bring value and purpose through every cup — serving with hospitality, excellence, and intentionality.</p>
      </div>
    </div>
    <div style={{maxWidth:620,margin:"0 auto",padding:"0 16px 80px"}}>
      <div className={mounted?"si":""} style={{background:"#fff",borderRadius:24,padding:"32px",marginTop:-32,boxShadow:"0 30px 80px rgba(0,0,0,.12)",border:"1px solid var(--border)"}}>
        <div style={{fontFamily:"'Poppins',sans-serif",fontSize:17,fontWeight:700,color:"var(--green)",marginBottom:14}}>📋 Quick Info</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:26}} className="g2">
          {info.map(x=><div key={x.l} className="ch" style={{background:"#fff",borderRadius:14,padding:"14px 16px",border:"1px solid var(--border)"}}><div style={{fontSize:20,marginBottom:6}}>{x.i}</div><div style={{color:"#888",fontSize:10,fontWeight:700,letterSpacing:".08em",textTransform:"uppercase",marginBottom:4}}>{x.l}</div><div style={{color:"#1a1a1a",fontSize:13,fontWeight:500,lineHeight:1.5}}>{x.v}</div></div>)}
        </div>
        <div style={{background:"linear-gradient(135deg,rgba(79,124,31,.4),rgba(79,124,31,.6))",borderRadius:16,padding:"20px 22px",marginBottom:24,border:"1px solid #b8d888"}}>
          <div style={{fontFamily:"'Poppins',sans-serif",fontSize:17,fontWeight:700,color:"var(--green)",marginBottom:10}}>Our Mission</div>
          <p style={{color:"rgba(255,255,255,.88)",fontSize:14,lineHeight:1.85,margin:0}}>The culture of Hope Coffee is one of serving. We exist to serve great coffee, our community, our customers, coffee farmers, the church, one another, those in need, and ultimately, Jesus Christ.</p>
        </div>
        <div style={{fontFamily:"'Poppins',sans-serif",fontSize:17,fontWeight:700,color:"var(--green)",marginBottom:14}}>🌿 Team Values</div>
        <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:26}}>
          {vals.map(v=><div key={v.t} className="ch" style={{background:"#fff",borderRadius:14,padding:"16px 18px",border:"1px solid var(--border)"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}><span style={{fontSize:20}}>{v.i}</span><div style={{fontFamily:"'Poppins',sans-serif",fontSize:16,color:"#e8f5d0",fontWeight:700}}>{v.t}</div></div><p style={{color:"rgba(79,124,31,.7)",fontSize:13,lineHeight:1.75,margin:0}}>{v.d}</p></div>)}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:26}} className="g2">
          {[{r:"Barista",p:"$11–13/hr",h:"10–36 hrs/wk",d:"Craft drinks, connect with guests, share the Hope Coffee story."},{r:"Shift Lead",p:"Competitive",h:"Full or Part Time",d:"Run shifts, support the team, uphold our values and standards."}].map(x=><div key={x.r} className="ch" style={{background:"linear-gradient(135deg,#3a6a18,#1e4008)",borderRadius:14,padding:"16px",border:"none"}}><div style={{fontFamily:"'Poppins',sans-serif",fontSize:17,color:"#e8f5d0",fontWeight:700,marginBottom:5,letterSpacing:"-.2px"}}>{x.r}</div><div style={{color:"rgba(255,255,255,.8)",fontSize:12,fontWeight:600,marginBottom:6}}>{x.p} · {x.h}</div><p style={{color:"var(--txtMid)",fontSize:12,lineHeight:1.65,margin:0}}>{x.d}</p></div>)}
        </div>
        <div style={{textAlign:"center",background:"rgba(79,124,31,.05)",borderRadius:16,padding:"22px",border:"1px solid var(--border)"}}><div style={{fontSize:36,marginBottom:8}}>🙌</div><h3 style={{fontFamily:"'Poppins',sans-serif",fontSize:22,color:"#e8f5d0",fontWeight:700,margin:"0 0 8px"}}>Ready to join the family?</h3><p style={{color:"rgba(79,124,31,.7)",fontSize:13,lineHeight:1.7,margin:"0 0 12px"}}>If you're passionate about making a difference with every cup — we'd love to hear from you.</p><div style={{color:"rgba(144,187,74,.6)",fontSize:12}}>(469) 518-1994 · melissa@hopecoffee.com</div></div>
      </div>
    </div>
  </div>);
}
...
function ManagerAuthGate({onAuth}){
  ...
  return(<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100%",background:"#f5f0e8"}}>
    <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at center,rgba(79,124,31,.12),transparent 70%)",pointerEvents:"none"}}/>
    <div className="si" style={{position:"relative",zIndex:1,background:"#fff",borderRadius:24,padding:"44px 38px",width:"min(400px,calc(100vw - 32px))",boxShadow:"0 30px 80px rgba(0,0,0,.12)",textAlign:"center",border:"1px solid var(--border)"}}>
      <div className="fl" style={{width:70,height:70,borderRadius:"50%",background:BG,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 22px",boxShadow:"0 8px 32px rgba(79,124,31,.4)"}}><Logo s={40}/></div>
      <h2 style={{fontFamily:"'Poppins',sans-serif",fontSize:24,color:"#e8f5d0",fontWeight:700,margin:"0 0 8px",letterSpacing:"-.4px"}}>Manager Access</h2>
      <p style={{color:"var(--txtMid)",fontSize:13,margin:"0 0 24px",lineHeight:1.65}}>Enter your authorized email and password to access the hiring dashboard.</p>
      <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:16}}>
        <input type="email" value={email} onChange={e=>setEmail(e.target.value.slice(0,200))} onKeyDown={e=>{if(e.key==="Enter")tryLogin();}} placeholder="manager@hopecoffee.com" disabled={locked} style={{width:"100%",background:"rgba(255,255,255,.03)",border:`1.5px solid ${err&&!locked?"rgba(239,68,68,.4)":"rgba(79,124,31,.15)"}`,borderRadius:12,padding:"13px 16px",color:"#1a1a1a",fontSize:14,outline:"none",textAlign:"center"}}/>
        <div style={{position:"relative"}}>
          <input type={show?"text":"password"} value={pass} onChange={e=>setPass(e.target.value.slice(0,100))} onKeyDown={e=>{if(e.key==="Enter")tryLogin();}} placeholder="Password" disabled={locked} style={{width:"100%",background:"rgba(255,255,255,.03)",border:`1.5px solid ${err&&!locked?"rgba(239,68,68,.4)":"rgba(79,124,31,.15)"}`,borderRadius:12,padding:"13px 48px 13px 16px",color:"#1a1a1a",fontSize:14,outline:"none",textAlign:"center"}}/>
          <button type="button" onClick={()=>setShow(s=>!s)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#999",cursor:"pointer",fontSize:16}}>{show?"🙈":"👁️"}</button>
        </div>
      </div>
      <button type="button" onClick={tryLogin} disabled={disabled} className="btn" style={{width:"100%",padding:"14px 24px",borderRadius:12,background:disabled?"rgba(79,124,31,.05)":BG,color:disabled?"rgba(144,187,74,.3)":"#fff",fontSize:15,fontWeight:700,cursor:disabled?"not-allowed":"pointer",marginBottom:14,border:"1px solid #b8d888"}}>
...
function ManagerDashboard({applicants,onStatusChange,onDelete}){
  ...
  return(<div className="dl" style={{display:"flex",height:"100%",background:"#f5f0e8",overflow:"hidden"}}>
    {/* PDF Modal */}
    {modal&&entry&&entry.resume_base64&&(<div onClick={()=>setModal(false)} style={{position:"fixed",inset:0,zIndex:9000,background:"rgba(0,0,0,.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:24,animation:"ovIn .3s ease both"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:20,width:"min(800px,100%)",maxHeight:"90vh",overflow:"hidden",border:"1px solid rgba(79,124,31,.2)",display:"flex",flexDirection:"column",animation:"scaleIn .3s cubic-bezier(.34,1.56,.64,1) both"}}>
        <div style={{padding:"16px 20px",borderBottom:"1px solid rgba(79,124,31,.1)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div><div style={{fontFamily:"'Poppins',sans-serif",fontSize:16,color:"#e8f5d0",fontWeight:700,letterSpacing:"-.2px"}}>Resume — {entry.full_name}</div><div style={{color:"#999",fontSize:12,marginTop:2}}>{entry.resume_file_name}</div></div>
          <div style={{display:"flex",gap:10}}>
            <button type="button" onClick={()=>downloadResume(entry)} className="btn" style={{padding:"8px 16px",borderRadius:10,background:BG,color:"#fff",fontSize:12,fontWeight:700,border:"1px solid rgba(79,124,31,.2)"}}>⬇ Download</button>
            <button type="button" onClick={()=>setModal(false)} className="btn" style={{padding:"8px 14px",borderRadius:10,background:"rgba(255,255,255,.04)",color:"var(--txtMid)",fontSize:12,border:"1px solid rgba(255,255,255,.08)"}}>✕ Close</button>
          </div>
        </div>
        <div style={{flex:1,overflow:"auto",padding:20}}>
          {entry.resume_media_type==="application/pdf"
            ...
            :<div style={{background:"rgba(255,255,255,.02)",borderRadius:12,padding:20,border:"1px solid var(--border)"}}><p style={{color:"var(--green)",fontSize:13,marginBottom:14}}>This file type can't be previewed inline. Download to view.</p><button type="button" onClick={()=>downloadResume(entry)} className="btn" style={{padding:"10px 20px",borderRadius:10,background:BG,color:"#fff",fontSize:13,fontWeight:700}}>⬇ Download Resume</button></div>}
        </div>
      </div>
    </div>)}

    {/* Sidebar */}
    <div className="ds" style={{width:300,flexShrink:0,background:"#faf6f0",borderRight:"1px solid #e0d8cc",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{background:"linear-gradient(165deg,#022c22,#065f46,#047857)",padding:"18px 16px",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:12}}>
          ...
          <div><div style={{fontFamily:"'Poppins',sans-serif",fontSize:16,color:"#e8f5d0",fontWeight:700}}>Hire4Hope</div><div style={{fontFamily:"'Poppins',sans-serif",color:"var(--green)",fontSize:12,marginTop:1}}>Manager Dashboard</div></div>
        </div>
        <div style={{background:"rgba(0,0,0,.12)",borderRadius:10,padding:"9px 13px",border:"1px solid rgba(255,255,255,.08)"}}>
          <div style={{fontFamily:"'Poppins',sans-serif",color:"var(--txtMid)",fontSize:13,fontWeight:600,letterSpacing:"-.1px"}}>{greet}! ☕</div>
          <div style={{color:"var(--txtMute)",fontSize:11,marginTop:1}}>{vis.length} applicant{vis.length!==1?"s":""}{cnts.New?` · ${cnts.New} new`:""}</div>
        </div>
      </div>
      {/* Stats */}
      <div style={{padding:"12px",borderBottom:"1px solid rgba(79,124,31,.06)",flexShrink:0}}>
      ...
      {/* Search */}
      <div style={{padding:"10px 12px 8px",borderBottom:"1px solid rgba(79,124,31,.06)",flexShrink:0}}>
        <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name or email…" style={{width:"100%",background:"rgba(255,255,255,.03)",border:"1px solid var(--border)",borderRadius:10,padding:"9px 12px",fontSize:13,color:"#1a1a1a",outline:"none"}}/>
      </div>
      {/* Filters */}
      <div style={{padding:"8px 12px",borderBottom:"1px solid rgba(79,124,31,.06)",flexShrink:0,display:"flex",flexWrap:"wrap",gap:5}}>
        {["All","New","Interview","Hired","Rejected"].map(s=><button key={s} type="button" onClick={()=>setFilter(s)} className="btn" style={{padding:"4px 11px",borderRadius:18,border:`1px solid ${filter===s?"rgba(79,124,31,.4)":"transparent"}`,background:filter===s?"rgba(79,124,31,.12)":"transparent",color:filter===s?"#8abb4a":"rgba(144,187,74,.4)",fontSize:11,fontWeight:filter===s?700:500}}>
          {s}{s!=="All"&&cnts[s]?` (${cnts[s]})`:""}</button>)}
      </div>
      {/* List */}
      <div style={{flex:1,overflowY:"auto",padding:"8px 10px 12px"}}>
        {filtered.length===0
          ?<p style={{color:"#bbb",fontSize:13,textAlign:"center",padding:"24px 12px"}}>{vis.length===0?"No applications yet.":"No matches found."}</p>
          :filtered.map((a,i)=>{const sg=stG(a.status);const active=sel===a.id;return(
            <button key={a.id} type="button" onClick={()=>setSel(a.id)} className="rh" style={{display:"block",width:"100%",textAlign:"left",padding:"11px 12px",borderRadius:12,cursor:"pointer",marginBottom:5,background:active?"rgba(79,124,31,.1)":"rgba(255,255,255,.02)",border:`1px solid ${active?"rgba(79,124,31,.35)":"rgba(255,255,255,.05)"}`,animation:`slideL .28s ease ${i*28}ms both`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                <span style={{color:"#1a1a1a",fontSize:13,fontWeight:600}}>{a.full_name}</span>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{color:"var(--txtMute)",fontSize:11}}>{a.position}</span>
    ...
    <div className="dd" style={{flex:1,overflowY:"auto",padding:"24px 28px"}}>
      {!entry
        ?<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",gap:12}}>
          ...
          <p style={{color:"var(--txtMute)",fontSize:14,textAlign:"center"}}>{vis.length===0?"No applications yet.":"Select an applicant to review their details."}</p>
        </div>
        :<div key={entry.id} style={{background:"#fff",borderRadius:22,padding:"22px 24px",boxShadow:"0 12px 40px rgba(0,0,0,.35)",animation:"fadeUp .38s ease both",border:"1px solid var(--border)"}}>
          {/* Header */}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:16,marginBottom:16}}>
            <div style={{display:"flex",gap:14,alignItems:"flex-start",flex:1}}>
              ...
              <div>
                <h2 style={{fontFamily:"'Poppins',sans-serif",fontSize:24,color:"#e8f5d0",fontWeight:700,margin:"0 0 6px"}}>{entry.full_name}</h2>
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:4}}>
                  ...
                  <span style={{color:"var(--green)",fontSize:12,fontWeight:600,background:"rgba(79,124,31,.08)",padding:"3px 12px",borderRadius:18,border:"1px solid #b8d888"}}>{entry.position}</span>
                ...
                <p style={{color:"#888",fontSize:12,margin:"0 0 2px"}}>{entry.email} · {entry.phone}</p>
                <p style={{fontFamily:"'Poppins',sans-serif",color:"#aaa",fontSize:12}}>Applied {new Date(entry.created_at).toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})}</p>
              </div>
            </div>
            {/* Score circle */}
            {entry.risk_score&&<div style={{textAlign:"center",flexShrink:0}}>
              <div style={{position:"relative",width:76,height:76}}>
                <svg width="76" height="76" viewBox="0 0 76 76" style={{transform:"rotate(-90deg)"}}>
                  <circle cx="38" cy="38" r="30" fill="none" stroke="rgba(255,255,255,.06)" strokeWidth="8"/>
                ...
                <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
                  ...
                  <span style={{color:"var(--txtMute)",fontSize:9,fontWeight:700}}>/10</span>
                </div>
              </div>
              <p style={{fontFamily:"'Poppins',sans-serif",color:"#888",fontSize:11,margin:"4px 0 0",fontWeight:700}}>{sLbl(entry.risk_score)}</p>
            </div>}
          </div>

          {/* AI summary */}
          {entry.ai_summary&&<div style={{background:"rgba(96,165,250,.07)",borderRadius:12,padding:"13px 16px",marginBottom:12,border:"1px solid rgba(96,165,250,.2)"}}>
          ...
          {/* Score breakdown */}
          {entry.score_breakdown?.length>0&&<div style={{background:"#fff",borderRadius:14,padding:"14px 18px",marginBottom:12,border:"1px solid #b8d888"}}>
            <div style={{fontFamily:"'Poppins',sans-serif",fontSize:13,fontWeight:700,color:"var(--green)",marginBottom:12}}>📊 Score Breakdown</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}} className="g2">
              {entry.score_breakdown.map(b=><div key={b.label} style={{background:"#fafafa",borderRadius:10,padding:"9px 12px",border:"1px solid var(--border)"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <span style={{color:"#1a1a1a",fontSize:11,fontWeight:600}}>{b.label}</span>
                  <span style={{color:"var(--green)",fontSize:11,fontWeight:700}}>{b.raw}/10 <span style={{color:"#aaa",fontWeight:400}}>({b.weight}%)</span></span>
                </div>
                <div style={{height:4,borderRadius:2,background:"rgba(79,124,31,.15)",overflow:"hidden"}}><div style={{height:"100%",borderRadius:2,background:`linear-gradient(90deg,${b.raw>=7?"#059669,#5a8a2e":b.raw>=5?"#d97706,#fbbf24":"#dc2626,#f87171"})`,width:`${b.raw*10}%`,transition:"width 1s ease"}}/></div>
                {b.reason&&<p style={{color:"#aaa",fontSize:10,margin:"3px 0 0",lineHeight:1.45}}>{b.reason}</p>}
              </div>)}
            </div>
          </div>}
          ...
          {/* Resume */}
          {entry.resume_base64
            ?<div style={{background:"rgba(79,124,31,.04)",borderRadius:12,padding:"13px 16px",marginBottom:12,border:"1px solid var(--border)"}}>
              <div style={{fontFamily:"'Poppins',sans-serif",fontSize:13,fontWeight:700,color:"var(--green)",marginBottom:10}}>📎 Resume File Uploaded</div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div><p style={{color:"#1a1a1a",fontSize:13,fontWeight:600,margin:0}}>{entry.resume_file_name||"Resume"}</p><p style={{color:"var(--txtMute)",fontSize:11,margin:"2px 0 0"}}>AI read this file for scoring</p></div>
                </div>
                <div style={{display:"flex",gap:8}}>
                  {entry.resume_media_type==="application/pdf"&&<button type="button" onClick={()=>setModal(true)} className="btn" style={{background:"rgba(79,124,31,.08)",border:"1px solid rgba(79,124,31,.2)",borderRadius:10,padding:"7px 14px",color:"var(--green)",fontSize:12,fontWeight:700}}>👁 View</button>}
                  <button type="button" onClick={()=>downloadResume(entry)} className="btn" style={{background:BG,border:"1px solid #b8d888",borderRadius:10,padding:"7px 14px",color:"#fff",fontSize:12,fontWeight:700}}>⬇ Download</button>
                </div>
              </div>
            </div>
            :entry.resume_text
              ?<div style={{background:"rgba(79,124,31,.04)",borderRadius:12,padding:"13px 16px",marginBottom:12,border:"1px solid var(--border)"}}>
                <div style={{fontFamily:"'Poppins',sans-serif",fontSize:13,fontWeight:700,color:"var(--green)",marginBottom:8}}>📋 Resume Summary</div>
                <p style={{color:"var(--txtMid)",fontSize:13,lineHeight:1.75,margin:0,whiteSpace:"pre-line"}}>{entry.resume_text}</p>
              </div>
              :null}

          {/* Details */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}} className="g2">
            {[{l:"Experience",v:`${entry.experience_years} yr${entry.experience_years!==1?"s":""}`},{l:"Availability",v:entry.availability}].map(x=><div key={x.l} style={{background:"#fafafa",borderRadius:10,padding:"10px 13px",border:"1px solid var(--border)"}}>
              <div style={{color:"var(--txtMute)",fontSize:10,fontWeight:600,letterSpacing:".08em",textTransform:"uppercase",marginBottom:3}}>{x.l}</div>
              <div style={{color:"#1a1a1a",fontSize:13,fontWeight:600}}>{x.v||"—"}</div>
            </div>)}
          </div>
          {[{l:"Background Notes",v:entry.background_notes,e:"📝"},{l:"Online Presence",v:entry.digital_footprint,e:"🔗"}].filter(x=>x.v).map(x=><div key={x.l} style={{background:"#fafafa",borderRadius:12,padding:"13px 16px",marginBottom:10,border:"1px solid var(--border)"}}>
            <div style={{fontFamily:"'Poppins',sans-serif",fontSize:13,fontWeight:700,color:"var(--green)",marginBottom:6}}>{x.e} {x.l}</div>
            <p style={{color:"var(--txtMid)",fontSize:13,lineHeight:1.75,margin:0,whiteSpace:"pre-line"}}>{x.v}</p>
          </div>)}

          {/* Status update */}
          <div style={{background:"#fff",borderRadius:16,padding:"16px 20px",marginTop:4,border:"1px solid #b8d888"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
              <div style={{fontFamily:"'Poppins',sans-serif",fontSize:14,fontWeight:700,color:"var(--green)"}}>Update Status</div>
              {statusMsg&&<div style={{fontSize:12,color:"var(--txtMid)",fontWeight:600,animation:"fadeUp .3s ease both"}}>{statusMsg}</div>}
            </div>
            <div style={{background:"rgba(96,165,250,.06)",borderRadius:10,padding:"9px 13px",marginBottom:12,border:"1px solid rgba(96,165,250,.15)"}}>
              <p style={{color:"#60a5fa",fontSize:12,margin:0,lineHeight:1.6}}>📧 <strong>Auto-email:</strong> Moving to Interview, Hired, or Rejected automatically emails the applicant.</p>
            </div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14}}>
              {[{l:"Interview",s:"Interview",g:"linear-gradient(165deg,#b45309,#78350f)",sh:"rgba(180,83,9,.35)"},{l:"Hired",s:"Hired",g:"linear-gradient(165deg,#047857,#022c22)",sh:"rgba(79,124,31,.35)"},{l:"Reject",s:"Rejected",g:"linear-gradient(165deg,#b91c1c,#7f1d1d)",sh:"rgba(185,28,28,.35)"},{l:"Reset",s:"New",g:"linear-gradient(165deg,#1d4ed8,#1e3a8a)",sh:"rgba(29,78,216,.35)"}].map(b=>{
            ...
            <div style={{borderTop:"1px solid rgba(79,124,31,.1)",paddingTop:12}}>
              {!confirmDel
                ...
                :<div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                  ...
                  <div style={{display:"flex",gap:7}}>
                    ...
                    <button type="button" onClick={()=>setConfirmDel(false)} className="btn" style={{padding:"6px 14px",borderRadius:9,border:"1px solid #b8d888",background:"transparent",color:"rgba(144,187,74,.6)",fontSize:12,fontWeight:600}}>Cancel</button>
                  </div>
                </div>}
            </div>
          </div>
...
export default function App(){
  ...
  if(!loaded)return(<div style={{height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f5f0e8"}}><style>{CSS}</style><div style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:16}}><div className="fl"><Logo s={50}/></div><p style={{fontFamily:"'Poppins',sans-serif",fontSize:22,color:"var(--green)"}}>Loading Hire4Hope…</p><div style={{width:40,height:3,borderRadius:2,background:"rgba(79,124,31,.15)",overflow:"hidden"}}><div style={{height:"100%",background:"linear-gradient(90deg,transparent,#7aab3a,transparent)",backgroundSize:"200% 100%",animation:"shimmer 1.5s linear infinite"}}/></div></div></div>);
  ...
  return(<div style={{height:"100vh",display:"flex",flexDirection:"column"}}>
    <style>{CSS}</style>
    <div style={{flexShrink:0,background:"#fff",borderBottom:"1px solid #e0d8cc",padding:"0 20px",display:"flex",alignItems:"center",height:54,gap:4,boxShadow:"0 4px 24px rgba(0,0,0,.12)"}}>
      <button type="button" onClick={()=>setPage("apply")} className="btn" style={{display:"flex",alignItems:"center",gap:9,marginRight:14,background:"none",border:"none",padding:"4px 8px",borderRadius:10,cursor:"pointer"}}>
        ...
        <div><div style={{fontFamily:"'Poppins',sans-serif",fontSize:15,fontWeight:700,lineHeight:1,background:"linear-gradient(135deg,#5a8a2e,#7aab3a)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Hire4Hope</div><div style={{fontFamily:"'Poppins',sans-serif",color:"#999",fontSize:10,lineHeight:1,marginTop:1}}>by Hope Coffee Melissa</div></div>
      </button>
      <div style={{width:1,height:20,background:"rgba(79,124,31,.2)",marginRight:8}}/>
      {navItems.map(p=><button key={p.id} type="button" onClick={()=>setPage(p.id)} className="btn" style={{display:"flex",alignItems:"center",gap:6,padding:"6px 14px",borderRadius:10,border:"none",background:page===p.id?"rgba(79,124,31,.08)":"transparent",color:page===p.id?"#5a8a2e":"rgba(79,124,31,.38)",fontSize:13,fontWeight:page===p.id?700:500,cursor:"pointer",borderBottom:`2px solid ${page===p.id?"#5a8a2e":"transparent"}`}}>{p.ic}<span className="nl">{p.l}</span></button>)}
      <button type="button" onClick={()=>setPage("manager")} className="btn" style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:5,padding:"5px 10px",borderRadius:9,border:`1px solid ${page==="manager"?"rgba(79,124,31,.3)":"rgba(79,124,31,.07)"}`,background:page==="manager"?"rgba(79,124,31,.08)":"transparent",color:page==="manager"?"#5a8a2e":"rgba(79,124,31,.2)",fontSize:11,fontWeight:500,cursor:"pointer"}}>
